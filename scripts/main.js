...

{
	...
	const EVENT = {
		RECEIVED: {
			...
			QUESTION_ANSWER: 'answerResult'
		}
	};
	
	self._lolPlugin = null;
	
	const getLolPlugin=function(){
		return self._lolPlugin;
	}

	const HANDLER_CLASS = class ppstudio_lolapi_DOMHandler extends self.DOMElementHandler
	{
		constructor(iRuntime)
		{
			...
			this._messageSent="";
			this._lolAPIState="";
			self._lolPlugin = this;
			//self["lol"]=this;
			
			self.addEventListener("message", this._LoLMessageHandler);
			
			//Listeners to the DOM events must be registered
			this.AddRuntimeMessageHandlers([
					["lol-api", d => this._LoLApi(d)],
					["loading-progress", d => this._LoadingProgress(d)],
					["alert", d => alert(d["msg"])]
				]
			);
		}		
		...
		_LoLApi(d) 
		{
			//messageName, payloadObj
			var messageName = d["messageName"],
				payloadObj = d["payloadObj"];
			
			this._payload = payloadObj;
			this._messageSent = messageName;
		...
		// Percent must be expressed in decimals, Eg. 0.5 = 50%, 1.0 = 100%
			this.LoLApi('loadingProgress', { "progress": p });	
			this._DebugConsoleLog("LoL API - LoadingProgress ==>"+p);
		}
		
		_DebugConsoleLog(msg){
			const d={
				"message":msg
			};
			getLolPlugin().PostToRuntime("debug",d);
		}

		_LoLMessageHandler(msg){
			let messageName = msg.data["messageName"];
			this._lolAPIState = messageName;
			let payload = msg.data["payload"]; 
			try {
				getLolPlugin()._payload = JSON.parse(payload);
				//this._payload = _payload;
			}
			catch(e){

				getLolPlugin()._lastError="Payload Parsing Error: "+e+"\r\n"+payload;

				var d={
					"event":"error",
					"msg":getLolPlugin()._lastError
				};
				getLolPlugin().PostToRuntime("lol-on-error",d);
				console.error(getLolPlugin()._lastError);
			}
			
			switch(messageName)
			{
				case EVENT.RECEIVED.START:
					//TO DO: Add event processing logic
					getLolPlugin()._DebugConsoleLog("LoL READY message received");
					getLolPlugin()._DebugConsoleLog(JSON.stringify(getLolPlugin()._payload));
					var d={
						"event":messageName,
						"lang":getLolPlugin()._payload["languageCode"]
					};
					getLolPlugin().PostToRuntime("lol-start",d);
					break;
					
				case EVENT.RECEIVED.PAUSE:
					//TO DO: Add event processing logic
					getLolPlugin()._DebugConsoleLog("LoL PAUSE request received");
					getLolPlugin()._DebugConsoleLog(JSON.stringify(getLolPlugin()._payload));
					var d={
						"event":messageName
					};
					getLolPlugin().PostToRuntime("lol-pause",d);
					break;

				case EVENT.RECEIVED.RESUME:
					//TO DO: Add event processing logic
					getLolPlugin()._DebugConsoleLog("LoL RESUME request received");
					getLolPlugin()._DebugConsoleLog(JSON.stringify(getLolPlugin()._payload));
					var d={
						"event":messageName
					};
					getLolPlugin().PostToRuntime("lol-resume",d);
					break;

				case EVENT.RECEIVED.LANGUAGE:
					getLolPlugin()._DebugConsoleLog("LoL LANGUAGE event received");
					getLolPlugin()._DebugConsoleLog(JSON.stringify(getLolPlugin()._payload));
					//getLolPlugin()._languageTexts=getLolPlugin()._payload;
					var d={
						"event":messageName,
						"payload":JSON.parse(payload)
					};
					getLolPlugin()._DebugConsoleLog(d["payload"]);					
					
					getLolPlugin().PostToRuntime("lol-lang",d);
					break;

				case EVENT.RECEIVED.INIT:
					getLolPlugin()._DebugConsoleLog("LoL INIT message received");
					getLolPlugin()._DebugConsoleLog(JSON.stringify(getLolPlugin()._payload));
					break;

				case EVENT.RECEIVED.QUESTIONS:
					getLolPlugin()._DebugConsoleLog("LoL QUESTIONS message received");
					getLolPlugin()._DebugConsoleLog(JSON.stringify(getLolPlugin()._payload));
					var d={
						"event":messageName,
						"payload":getLolPlugin()._payload
					};
					getLolPlugin().PostToRuntime("lol-question",d);
					break;
				case EVENT.RECEIVED.LOAD_STATE:
					getLolPlugin()._DebugConsoleLog("LoL LOAD STATE message received");
					getLolPlugin()._DebugConsoleLog(JSON.stringify(getLolPlugin()._payload));
					...
					getLolPlugin().PostToRuntime("load-state",d);

					break;
				case EVENT.RECEIVED.SAVE_STATE_RESULT:
					getLolPlugin()._DebugConsoleLog("LoL SAVE STATE RESULT message received");
					getLolPlugin()._DebugConsoleLog(JSON.stringify(getLolPlugin()._payload));
					...
					getLolPlugin().PostToRuntime("save-state-result",d);
					break;
				case EVENT.RECEIVED.QUESTION_ANSWER:
					getLolPlugin()._DebugConsoleLog("LoL SAVE STATE RESULT message received");
					getLolPlugin()._DebugConsoleLog(JSON.stringify(getLolPlugin()._payload));
					...
					getLolPlugin().PostToRuntime("question-answered",d);
					break;
				default:
					getLolPlugin()._DebugConsoleLog("LoL UNHANDLED MESSAGE");
					getLolPlugin()._DebugConsoleLog(JSON.stringify(msg));
					getLolPlugin()._lastError = "LoL UNHANDLED MESSAGE: "+messageName+"\n"+JSON.stringify(msg);
					var d={
						"event":"error",
						"msg":getLolPlugin()._lastError
					};
